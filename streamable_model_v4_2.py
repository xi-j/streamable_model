...
class StreamableModel(nn.Module):
    ...
    class StreamableConv1D(nn.Module):
        ...
        bad_input_length: maximum number of bad input samples in stable condition
        layer_id: index of the layer
        ''' 
        def __init__(self, layer:nn.Conv1d, input_length:int, bad_input_length:int, layer_id:int):
            super().__init__()
            ...
            # maximum number of bad input samples in stable condition
        ...
        def forward(self, input_buffer):
            ...
            #  |        last_bad_input_length     | input_length - self.bad_input_length - bad_input_length |  bad_input_length |
            ...
            bad_input_length = min(input_length, self.bad_input_length)

            prev_length = self.input_counter - self.output_counter * self.stride + self.padding + self.last_bad_input_length 
            new_input_length = input_length - self.last_bad_input_length
            self.last_bad_input_length = bad_input_length

            full_length = prev_length + new_input_length + self.padding
            good_length = full_length - bad_input_length - self.padding

            '''
            print("Layer {} bad_input_length: ".format(self.layer_id), bad_input_length)
            print("Layer {} input_length: ".format(self.layer_id), input_length)
            print("Layer {} prev_length: ".format(self.layer_id), prev_length)
            print("Layer {} full_length: ".format(self.layer_id), full_length)
            print("Layer {} good_length: ".format(self.layer_id), good_length)
            print("Layer {} new_input_length: ".format(self.layer_id), new_input_length)
            '''

            # buffer initially
            #  |              old good samples           |     old bad samples   | paddings |
            #                                            | last_bad_input_length |
            ...
            #  |   ...   | last_bad_input_length |            new_input_length - bad_input_length          |  bad_input_length | paddings |
    ...
    class StreambleConstInterpolation(nn.Module):
        ...
        ''' 
        def __init__(self, layer:nn.Upsample, input_length:int, bad_input_length:int, layer_id:int):
            super().__init__()
            self.input_length = input_length
            self.bad_input_length = bad_input_length
            self.layer_id = layer_id
        ...
        ''' 
        def forward(self, input_buffer):
            return self.interp(input_buffer)

    '''
    streamable activation e.g. ReLU, PReLU
    '''
    class StreamableActivation(nn.Module):
        def __init__(self, layer:nn.ReLU, input_length:int, bad_input_length:int, layer_id:int):
            super().__init__()
            self.input_length = input_length
            self.bad_input_length = bad_input_length
            self.layer_id = layer_id
        ...
        ''' 
        def forward(self, input_buffer):
            return self.activation(input_buffer)
    ...
    class StreamableConvT1D(nn.Module):
        ...
        layer_id: index of the layer
        ''' 
        def __init__(self, layer:nn.ConvTranspose1d, input_length:int, bad_input_length:int, layer_id:int):
            super().__init__()
        ...
        def forward(self, input_buffer):
            ...
            corrected_input_length = self.last_bad_input_length
            self.last_bad_input_length = bad_input_length
    ...
    '''
    def __init__(self, buffer_length:int, layers:nn.Sequential, shortcutDict = None):  
        super().__init__()
        ...
        for i in range(0, self.num_layers):
            ...
            if isinstance(layers[i], nn.Conv1d):
                ...
                self.streamable_layers.append(self.StreamableConv1D(layers[i], bad_input_length + new_input_length, bad_input_length, i))
            ...
            # streamalize nn.ConvTranspose1d
                ...
                self.streamable_layers.append(self.StreamableConvT1D(layers[i], bad_input_length + new_input_length, bad_input_length, i))
            ...
            # streamalize nn.Upsample with mode = 'nearest'
                ...
                self.streamable_layers.append(self.StreambleConstInterpolation(layers[i], bad_input_length + new_input_length, bad_input_length, i))
            ...
            # streamalize activations 
                ...
                self.streamable_layers.append(self.StreamableActivation(layers[i], bad_input_length + new_input_length, bad_input_length, i))

        self.bad_output_length = self.bad_samples_num[self.num_layers]
        self.new_output_length = self.new_samples_num[self.num_layers]
        ...
        if self.residual_flag:
            ...
            for i in self.shortcutDict.keys():
                ...
                self.buffer_img[j] = torch.zeros(1, in_channels, length)
    ...
    def forward(self, buffer):
        ...
        for i in range(self.num_layers):
            buffer = self.streamable_layers[i](buffer)

        bad_output_length = min(buffer.shape[2], self.bad_output_length)
    ...
    def residual_forward(self, buffer):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.shortcutDict:
                ...
                buffer = self.streamable_layers[i](buffer + residual)
            ...
            if i in self.buffer_img:
                ...
                bad_residual_length = self.bad_samples_num[i + 1]
...
if __name__ == '__main__':
    ...
    my_model = []
    my_model.append(nn.Conv1d(1, 256, kernel_size = 21, stride = 10, padding = 21//2))
    my_model.append(nn.Conv1d(256, 256, kernel_size = 7, stride = 3, padding = 7//2))
    for i in range(20):
        my_model.append(nn.Conv1d(256, 256, kernel_size = 3, stride = 1, padding = 3//2, groups = 256))
    my_model.append(nn.ConvTranspose1d(256, 256, kernel_size = 21, stride = 10, padding = 21//2, output_padding = 10 - 1))
    my_model= nn.Sequential(*my_model)
    ...
    residual_connection = {4 : 1,  7 : 4, 10 : 7, 13 : 10, 16 : 13}
    ...
    for i in range(buffer_num):
        ...
        mis_count = torch.sum((abs(output_buffer_ref[:, 0, :] - output_buffer[:, 0, :]) < 1e-5)).item()
        ...
        if equal:
            ...
            print("WRONG OUTPUT!!!")
            print("WRONG OUTPUT!!!")
            print("difference by {} samples.".format(mis_count))
